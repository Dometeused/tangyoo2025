      ...
      >
        ...
        {(theme === "anniversary" || theme === "birthday") && (
          <div
            ...
            onClick={handleInteraction}
          >
            {/* Gold dust particles — more on dark bg */}
            {Array.from({ length: 60 }).map((_, i) => (
              <GoldDust key={i} delay={i * 0.08} />
            ))}

            {/* Soft glow */}
            <motion.div className="absolute pointer-events-none rounded-full"
              style={{ width: 360, height: 360, top: "50%", left: "50%", transform: "translate(-50%,-60%)",
            ...
            <div style={{ position: "relative", width: 240, height: 130 }}>
              <motion.svg width="240" height="130" viewBox="0 0 240 130"
                style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }}
                animate={boxOpen ? { opacity: 0 } : {}} transition={boxOpen ? { duration: 0.18 } : {}}>
              ...
              {/* Left balloon */}
              <motion.div style={{ position: "absolute", left: 20, top: 16 }}
                ...
                <svg width="62" height="70" viewBox="0 0 62 70" style={{ filter: "drop-shadow(0 4px 12px rgba(210,140,80,0.5))" }}>
                  <defs><radialGradient id="avRG2" cx="35%" cy="28%" r="65%"><stop offset="0%" stopColor="#ffd8c8"/><stop offset="100%" stopColor="#c88858"/></radialGradient></defs>
                  <ellipse cx="31" cy="27" rx="23" ry="27" fill="url(#avRG2)"/>
                  <ellipse cx="22" cy="17" rx="8" ry="10" fill="rgba(255,255,255,0.34)"/>
                  <path d="M28 54 L31 60 L34 54 Z" fill="#c88858"/>
                </svg>
              </motion.div>
              {/* Center balloon */}
              <motion.div style={{ position: "absolute", left: 76, top: 3 }}
                ...
                <svg width="66" height="74" viewBox="0 0 66 74" style={{ filter: "drop-shadow(0 4px 16px rgba(230,190,30,0.6))" }}>
                  <defs><radialGradient id="avGold2" cx="35%" cy="28%" r="65%"><stop offset="0%" stopColor="#fff8c0"/><stop offset="100%" stopColor="#e8c040"/></radialGradient></defs>
                  <ellipse cx="33" cy="29" rx="25" ry="29" fill="url(#avGold2)"/>
                  <ellipse cx="23" cy="18" rx="9" ry="11" fill="rgba(255,255,255,0.36)"/>
                  <path d="M30 58 L33 65 L36 58 Z" fill="#e8c040"/>
                </svg>
              </motion.div>
              {/* Right balloon */}
              <motion.div style={{ position: "absolute", left: 144, top: 12 }}
                ...
                <svg width="60" height="68" viewBox="0 0 60 68" style={{ filter: "drop-shadow(0 4px 12px rgba(195,160,60,0.5))" }}>
                  <defs><radialGradient id="avChamp2" cx="35%" cy="28%" r="65%"><stop offset="0%" stopColor="#f0e8d0"/><stop offset="100%" stopColor="#c8a858"/></radialGradient></defs>
                  <ellipse cx="30" cy="27" rx="22" ry="27" fill="url(#avChamp2)"/>
            ...
            {/* ── Gift Box ── */}
            <motion.div className="relative" style={{ width: 164, height: 176, marginTop: -20 }}
              animate={boxOpen ? { scale: [1, 1.08, 0], opacity: [1, 1, 0] } : {}}
              transition={{ duration: 0.75, ease: [0.4, 0, 0.2, 1] }}
            >
              {/* Lid */}
              <motion.div className="absolute top-0 left-0 w-full z-20"
                style={{ height: 54, transformOrigin: "top center" }}
                animate={boxOpen ? { y: -90, rotateX: 120, opacity: 0 } : { y: [0, -7, 0] }}
                transition={boxOpen ? { duration: 0.5, ease: [0.4, 0, 1, 1] } : { repeat: Infinity, duration: 2.8, ease: "easeInOut" }}
              >
                <div className="w-full h-full relative overflow-hidden" style={{
                  borderRadius: "10px 10px 0 0", background: boxColor.lid,
                  ...
                  <motion.div className="absolute inset-y-0 w-10 pointer-events-none"
                    ...
                    animate={{ x: [-60, 200] }} transition={{ repeat: Infinity, duration: 2.2, ease: "easeInOut", repeatDelay: 1.2 }}
                  />
                  <div className="absolute top-0 left-0 right-0 h-[1px]" style={{ background: `linear-gradient(90deg, transparent, ${boxColor.topLine}, transparent)` }} />
                  <div className="absolute left-1/2 -translate-x-1/2 w-[14px] h-full" style={{ background: "linear-gradient(180deg, #e8c060, #f8e080, #e8c060)", opacity: 0.95 }} />
                  <div className="absolute left-1/2 -translate-x-1/2 w-[3px] h-full" style={{ background: "rgba(255,255,255,0.45)", marginLeft: -1 }} />
                </div>
                {/* Bow */}
                <div className="absolute left-1/2 -translate-x-1/2" style={{ top: -48, width: 100, height: 56 }}>
                  <svg viewBox="0 0 100 56" fill="none" style={{ overflow: "visible" }}>
                    <defs>
                      <linearGradient id="bowG2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#fde080"/><stop offset="45%" stopColor="#e8c050"/><stop offset="100%" stopColor="#b89030"/></linearGradient>
                      <linearGradient id="bowDark2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#c9a040"/><stop offset="100%" stopColor="#806010"/></linearGradient>
                    </defs>
                    <path d="M50 30 C42 18, 10 8, 8 24 C6 38, 36 42, 50 30Z" fill="url(#bowG2)"/>
                    <path d="M50 30 C44 34, 18 40, 8 24 C6 38, 36 42, 50 30Z" fill="url(#bowDark2)" opacity="0.5"/>
                    <path d="M50 30 C58 18, 90 8, 92 24 C94 38, 64 42, 50 30Z" fill="url(#bowG2)"/>
                    <path d="M50 30 C56 34, 82 40, 92 24 C94 38, 64 42, 50 30Z" fill="url(#bowDark2)" opacity="0.5"/>
                    <path d="M50 32 C43 38, 32 48, 26 56" stroke="url(#bowG2)" strokeWidth="6" strokeLinecap="round"/>
                    <path d="M50 32 C57 38, 68 48, 74 56" stroke="url(#bowG2)" strokeWidth="6" strokeLinecap="round"/>
              ...
              <div className="absolute w-full overflow-hidden" style={{
                top: 50, height: 126, borderRadius: "0 0 12px 12px", background: boxColor.body,
                boxShadow: `0 20px 60px ${boxColor.shadow}, inset 0 1px 0 ${boxColor.shimmer}`,
              }}>
                <motion.div className="absolute inset-y-0 w-14 pointer-events-none"
                  style={{ background: `linear-gradient(90deg, transparent, ${boxColor.shimmer}, transparent)`, skewX: "-15deg" }}
                  animate={{ x: [-80, 220] }} transition={{ repeat: Infinity, duration: 2.5, ease: "easeInOut", repeatDelay: 0.8, delay: 0.5 }}
                />
                <div className="absolute left-1/2 -translate-x-1/2 w-[14px] h-full" style={{ background: "linear-gradient(180deg, #e8c060, #f8e080, #e8c060)", opacity: 0.9 }} />
                <div className="absolute left-1/2 -translate-x-1/2 w-[3px] h-full" style={{ background: "rgba(255,255,255,0.4)", marginLeft: -1 }} />
                <div className="absolute top-0 left-0 w-[1px] h-full" style={{ background: `linear-gradient(180deg, ${boxColor.topLine}, transparent)` }} />
                <AnimatePresence>
                  {boxOpen && (
                    <motion.div className="absolute inset-0"
                      style={{ background: "radial-gradient(circle at 50% 20%, rgba(255,220,80,0.98) 0%, rgba(255,140,20,0.6) 40%, transparent 70%)", borderRadius: "0 0 12px 12px" }}
              ...
              {boxOpen && (
                <>
                  ...
                  {Array.from({ length: 20 }).map((_, i) => (
                    <GoldRay key={i} angle={i * 18} delay={0.03 + i * 0.015} />
                  ))}
                  {/* 60 sparks — 2x more */}
                  {Array.from({ length: 60 }).map((_, i) => (
                    <GoldSpark key={i} delay={0.03 + Math.random() * 0.12} />
                  ))}
                  {/* 3 shockwave rings */}
                  {[
                    { size: 340, dur: 0.7, color: "rgba(255,210,0,0.8)", delay: 0 },
                    { size: 520, dur: 1.0, color: "rgba(255,180,0,0.5)", delay: 0.08 },
                    { size: 700, dur: 1.3, color: "rgba(232,204,112,0.3)", delay: 0.18 },
                  ].map((ring, i) => (
                    <motion.div key={i} className="absolute pointer-events-none rounded-full"
                      style={{ border: `${i === 0 ? 3 : i === 1 ? 2 : 1}px solid ${ring.color}`,
                  ...
                  <motion.div className="absolute inset-0 pointer-events-none"
                    style={{ background: "radial-gradient(circle, rgba(255,240,120,0.7) 0%, rgba(255,180,0,0.4) 30%, transparent 60%)" }}
                    initial={{ opacity: 0, scale: 0.2 }}
                    animate={{ opacity: [0, 1, 1, 0], scale: [0.2, 0.8, 1.4, 2.0] }}
                    transition={{ duration: 1.2, ease: "easeOut", times: [0, 0.2, 0.5, 1] }}
                  />
                  {/* Screen-fill flash */}
                  <motion.div className="absolute inset-0 pointer-events-none"
                    style={{ background: "rgba(255,210,50,0.18)" }}
                    initial={{ opacity: 0 }}
            ...
            <AnimatePresence>
              {!isOpening && (
                ...
                >
                  <motion.p className="font-kanit font-light tracking-[0.35em] text-xs"
                    style={{ color: "rgba(232,204,112,0.7)" }}
